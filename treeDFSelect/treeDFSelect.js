...
Tree.prototype.DFSelect = function(filter) { // this is what i call KISS
  var result = [];
  var depth = 0;
  ...
  function looper(tree,depth) {
    for(var i = 0; i < tree.children.length; i++){
      ...
      if(filter(tree.children[i].value,depth))
        ...
        looper(tree.children[i],depth+1)
    }
  }
  looper(this,1)
  return result 
...
Tree.prototype.DFSelect = function(filter, depth = 0,result = []) { //default values are the best   
  if(filter(this.value,depth))
    result.push(this.value)
